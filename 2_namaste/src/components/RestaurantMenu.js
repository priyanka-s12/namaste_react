...
function RestaurantMenu() {
  ...
  return (
    <div>
      <div className="w-6/12 bg-gray-50 mx-auto mt-3">
        <h1 className="font-bold my-6 text-2xl">{name}</h1>
        <p className="font-bold">Area: {areaName}</p>
        <p className="font-bold">Cuisines: {cuisines?.join(', ')}</p>
        <p className="font-bold">Rating: {avgRating}</p>
        <hr className="my-2" />
        <h2 className="text-xl font-bold my-3">Menu</h2>
      </div>
      {/* <ul>
        <li>Biryani</li>
        <li>Pulao</li>
      ...
      {/* restaurant menu categories,  “If this category is already open, close it. Otherwise, open this category.” */}
      {/* You click on... showIndex before click showIndex after click Result index
      = 2 null 2 Open #2 index = 2 2 null Close #2 index = 1 2 1 Open #1 */}
      {restMenu?.map((category, index) =>
        category?.type === 'item' ? (
          <ItemCategory
            key={category?.title}
            data={category}
